    ...
    def _update_centroid_location(self):
        ...
        for centroid, belongings in self.cluster_result.items():
            if not self.centroid_stable_flag.get(centroid):
                ...
                self.centroid_info[centroid] = [sum(i) / len(i) for i in zip(*temp_list)] 
...
if __name__ == '__main__':
    ...
    #inputPath = "../data/test5"
    #clusters = int("15")
    #outputClusters = "../out/cluster5.json"
    #outputfilePath = "../out/intermediate5.csv"

    inputPath = sys.argv[1]
    clusters = int(sys.argv[2])
    outputClusters = sys.argv[3]
    ...
    for index, path in enumerate(sorted(os.listdir(inputPath))):
        dataFile = ''.join(input_csv_dir_path + "/" + path)
        dataRDD = sc.textFile(data_file_path).map(lambda row: row.split(",")) \
            .map(lambda kvs: (int(kvs[0]), list(map(eval, kvs[1:]))))

        if index == 0:
            total_length = dataRDD.count()
            first_N = 10000 if total_length > 10000 else int(total_length * 0.1)
            sample_data = dataRDD.filter(lambda kv: kv[0] < first_N).collectAsMap()
            ds_center, ds_stat, ds_cluster = KMeans(k=clusters, max_iterations=5).fit(sample_data)
            discard_set.init(ds_center, ds_stat, ds_cluster)
            rest_data = dataRDD.filter(lambda kv: kv[0] >= first_N).collectAsMap()
            center, stat, cluster = KMeans(k=clusters * 3, max_iterations=3).fit(rest_data)
            ...
            center, stat, cluster = KMeans(k=clusters * 3, max_iterations=5) \
    ...
    intermediate_records.export(outputfilePath)
    exportClusterResult(DS_obj=discard_set,
                        CS_obj=compression_set,
                        RS_obj=retained_set,
                        output_file_path=outputClusters)
